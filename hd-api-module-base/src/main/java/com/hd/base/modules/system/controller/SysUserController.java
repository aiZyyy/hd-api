package com.hd.base.modules.system.controller;
...
import com.hd.base.modules.system.entity.*;
import com.hd.base.modules.system.model.DepartIdModel;
import com.hd.base.modules.system.model.SysUserSysDepartModel;
import com.hd.base.modules.system.service.*;
import com.hd.base.modules.system.util.TreesUtil;
import com.hd.base.modules.system.vo.SysDepartUsersVO;
import com.hd.base.modules.system.vo.SysUserRoleVO;
import com.hd.base.modules.system.vo.TreeSelectsDataVO;
import com.hd.common.util.ImportExcelUtil;
import com.hd.common.util.PasswordUtil;
...
//                json.put("avatar", item.getSysUser().getAvatar());
     ...
     * 得到用户部门树
     */
    @AutoLog(value = "得到用户部门树==用于短信发送")
    @GetMapping(value = "/getDeptUserTree")
    @ApiOperation(value = "得到用户部门树==用于短信发送", notes = "得到用户部门树==用于短信发送")
    public Result<List<TreeSelectsDataVO>> getDeptUserTree(){
        Result<List<TreeSelectsDataVO>> result = new Result<>();
        //根节点存放list
        List<TreeSelectsDataVO> rootList = Lists.newArrayList();
        //其他节点存放list
        List<TreeSelectsDataVO> bodyList = Lists.newArrayList();

        //获取所有节点(一条sql解决)
        bodyList = sysDepartService.selectBodyList();
//        for (TreeSelectsDataVO vo:bodyList){
//            vo.setKey(UUID.randomUUID().toString().replace("-", ""));
//        }

        //获取根节点
        rootList = sysDepartService.selectRootList();
//        for (TreeSelectsDataVO vo:rootList){
//            vo.setKey(UUID.randomUUID().toString().replace("-", ""));
//        }
        //组装数据
        TreesUtil.setTreeData(rootList,bodyList);
        List<TreeSelectsDataVO> resultTree = TreesUtil.getTree();
//        System.out.println(JSONObject.toJSON(resultTree));
        result.setSuccess(true);
        result.setMessage("查找成功");
        result.setResult(resultTree);
        return result;
    }
}
