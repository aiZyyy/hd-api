...
import com.hd.base.modules.system.model.SysCategoryTree;
...
import com.hd.common.system.vo.DictModel;
...
public class SysCategoryController {
    ...
    public Result<List<SysCategoryTree>> list() {
        long start = System.currentTimeMillis();
        Result<List<SysCategoryTree>> result = new Result<>();
        try {
            LambdaQueryWrapper<SysCategory> query = new LambdaQueryWrapper<SysCategory>();
            query.orderByAsc(SysCategory::getSortNo);
            List<SysCategory> list = sysCategoryService.list(query);
            List<SysCategoryTree> treeList = new ArrayList<>();
    ...
    private void getTreeList(List<SysCategoryTree> treeList, List<SysCategory> metaList, SysCategoryTree temp) {
        for (SysCategory sysCategory : metaList) {
            String tempPid = sysCategory.getPid();
            SysCategoryTree tree = new SysCategoryTree(sysCategory);
    ...
    private void getTreeModelList(List<TreeModel> treeList, List<SysCategory> metaList, TreeModel temp) {
        for (SysCategory sysCategory : metaList) {
            String tempPid = sysCategory.getPid();
            TreeModel tree = new TreeModel(sysCategory);
            if (temp == null && oConvertUtils.isEmpty(tempPid)) {
                treeList.add(tree);
                if (!tree.getIsLeaf()) {
    ...
    public Result<SysCategory> add(@RequestBody SysCategory sysCategory) {
        ...
        try {
            ...
            log.error(e.getMessage(), e);
            result.error500("操作失败,请确认同组别中是否已存在该分组编码");
        }
        return result;
    }
    ...
    public Result<SysCategory> delete(@RequestParam(name = "id", required = true) String id) {
        ...
        if (sysCategory == null) {
            ...
            try {
                sysCategoryService.deleteGroup(id);
                result.success("删除成功!");
            } catch (Exception e) {
                log.error(e.getMessage(), e);
